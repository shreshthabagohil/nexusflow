...
import { getShipment, getMockShipments } from "../services/api";
import RerouteModal from "./RerouteModal";
import { NexusHeader } from "./Dashboard";

const BACKEND_URL = import.meta.env.VITE_BACKEND_URL ?? "http://localhost:8000";

// Derive plausible ML features from shipment data when backend is offline
function deriveMockFeatures(shipment) {
  const score = Number(shipment?.risk_score ?? 50);
  const seed  = (shipment?.id ?? "X").split("").reduce((a, c) => a + c.charCodeAt(0), 0);
  const r     = (offset) => Math.round((seed * 7 + offset) % 100) / 100; // deterministic 0-1

  const weather   = Math.min(1, score / 100 * 0.75 + r(3)  * 0.3);
  const origCong  = Math.min(1, score / 100 * 0.9  + r(7)  * 0.15);
  const destCong  = Math.min(1, score / 100 * 0.55 + r(13) * 0.35);
  const ontime    = Math.max(0.3, 1 - score / 100 * 0.6);
  const priority  = 1 + (seed % 9);
  const daysEta   = shipment?.eta
    ? Math.max(0, Math.ceil((new Date(shipment.eta) - Date.now()) / 86400000))
    : 14 + (seed % 20);
  const distance  = 3000 + (seed % 30) * 300;

  return {
    weather_severity:      +weather.toFixed(2),
    origin_congestion:     +origCong.toFixed(2),
    dest_congestion:       +destCong.toFixed(2),
    carrier_ontime_rate:   +ontime.toFixed(2),
    cargo_priority_weight: priority,
    days_until_eta:        daysEta,
...
export default function ShipmentDetail() {
  ...
  useEffect(() => {
    // ── Instant render: find mock data synchronously, show it right away ──
    const mockList   = getMockShipments();
    const mockMatch  = mockList.find((s) => s.id === id) ?? null;
    if (mockMatch) {
      setShipment(mockMatch);
      setFeatures(deriveMockFeatures(mockMatch));
      setLoading(false);
    } else {
      setLoading(true);
    }

    // ── Background: silently try backend for richer live data ──
    const featuresWithTimeout = Promise.race([
    ...
    Promise.all([
      ...
      if (s) setShipment(s);
      if (f)  setFeatures(f);
      // if backend had no features, keep the derived ones already on screen
            ...
            <div style={{
              ...
              {shipment.top_risk_factors.map((rf, i) => {
                ...
                return (
                  <div key={i} style={{
                    flex:         "1 1 220px",
                    background:   isIncrease ? "rgba(217,119,6,0.1)" : "rgba(16,185,129,0.08)",
                    border:       `1px solid ${isIncrease ? "rgba(245,158,11,0.28)" : "rgba(16,185,129,0.2)"}`,
                    borderRadius: 8,
                    ...
                    <span style={{ fontSize: "1.1rem", flexShrink: 0, color: isIncrease ? "#f59e0b" : "#34d399" }}>
                      {isIncrease ? "⬆" : "⬇"}
                    </span>
                    <div>
                      <div style={{
                        fontSize:   "0.82rem",
                        fontWeight: 600,
                        color:      isIncrease ? "#fbbf24" : "#34d399",
                      }}>
                        {rf.factor}
                      </div>
                      <div style={{
