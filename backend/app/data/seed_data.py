...
Responsibilities:
  ...
  - SHIPMENTS_SEED      — 53 shipments (NX1001-NX1050 + S001/S002/S003 for demos)
...
# Path to the 500-shipment dataset generated by data/generate_synthetic_data.py
_SHIPMENTS_JSON = Path(__file__).resolve().parents[3] / "data" / "shipments.json"
# Fallback: also check /app/data/shipments.json (mounted in Docker)
_SHIPMENTS_JSON_DOCKER = Path("/app/data/shipments.json")
...
# Public seed list — all 53 shipments, fully enriched
SHIPMENTS_SEED: list[dict[str, Any]] = [_enrich(s) for s in _RAW_SHIPMENTS]
...
async def seed_redis(redis) -> None:  # type: ignore[type-arg]
    ...
    # ── Shipments: prefer 500-entry shipments.json, fall back to SHIPMENTS_SEED ──
    shipments_to_seed: list[dict[str, Any]] = []
    for candidate in (_SHIPMENTS_JSON_DOCKER, _SHIPMENTS_JSON):
        if candidate.exists():
            try:
                loaded = json.loads(candidate.read_text())
                if isinstance(loaded, list) and loaded:
                    shipments_to_seed = loaded
                    logger.info("seed_redis: loaded %d shipments from %s", len(loaded), candidate)
                    break
            except Exception as exc:
                logger.warning("seed_redis: could not load %s: %s", candidate, exc)

    if not shipments_to_seed:
        shipments_to_seed = list(SHIPMENTS_SEED)
        logger.info("seed_redis: using built-in SHIPMENTS_SEED (%d shipments)", len(shipments_to_seed))
    ...
    await pipe.execute()
    logger.info(
        "Redis seeded: %d shipments, %d weather ports, %d carriers.",
        len(shipments_to_seed),
        len(DEFAULT_WEATHER),
